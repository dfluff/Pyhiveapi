...
api_device_list_all = {}
api_device_list_all = HiveAPI.initialise_api(
    HiveUserName, HivePassword, UpdateIntervalInMinutes
)
...
if len(api_device_list_all) == 0:
    ...
    for adevice in api_device_list_sensor:
        print(
            "sensor :: "
            + str(adevice["Hive_NodeName"])
            + " : "
            + str(adevice["HA_DeviceType"])
            + " : "
            + str(adevice["Hive_NodeID"])
        )
    ...
    for adevice in api_device_list_climate:
        ...
        if (
            str(adevice["HA_DeviceType"]) == "HotWater"
            and str(adevice["Hive_NodeName"]) == "None"
        ):
            HotwaterNodeID = str(adevice["Hive_NodeID"])
        if (
            str(adevice["HA_DeviceType"]) == "Heating"
            and str(adevice["Hive_NodeName"]) == "None"
        ):
            Heating_NodeID_SingleZone = str(adevice["Hive_NodeID"])
        if (
            str(adevice["HA_DeviceType"]) == "Heating"
            and str(adevice["Hive_NodeName"]) == Heating_Name_Zone_1
        ):
            Heating_NodeID_Zone_1 = str(adevice["Hive_NodeID"])
        if (
            str(adevice["HA_DeviceType"]) == "Heating"
            and str(adevice["Hive_NodeName"]) == Heating_Name_Zone_2
        ):
    ...
    if Heating_NodeID_SingleZone != "":
        ...
        print(
            "Boost ends in "
            + str(heating.get_boost_time(Heating_NodeID_SingleZone))
            + " minutes"
        )
